...
def modify():
    ...
    for i in ticket:
        if i.numberTicket==numberToSearch:
            ...
            obj=Ticket(numberToSearch,nameClient,idClient,quantity,movieName,weekDay)
            ticket.remove(i)
            ticket.append(obj)
            print("")
...
def readTicketData():
    ...
    print('Price base it`5  5$ per ticket')
    print('')
    ...
    if weekDay=="Monday" or weekDay=="monday":
        price=quantity*(price-(price*(50/100)))
    elif weekDay=="Thursday" or weekDay=="thursday":
        price=quantity*(price-(price*(30/100)))
    else:
        price=quantity*price
    if not searchinList(numberTicket):
        obj=Ticket(numberTicket,nameClient,idClient,quantity,movieName,weekDay)
        ticket.append(obj)
        print("")
        print("Ticket Added")
